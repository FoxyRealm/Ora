...
import { ArrowDownRight, ArrowUpRight, Banknote, BookOpenCheck, BriefcaseBusiness, Building2, CalendarClock, CheckCircle2, ChevronRight, CircleDollarSign, ClipboardList, CreditCard, FileCheck2, FilePlus2, Files, Hourglass, Landmark, LayoutDashboard, PackageSearch, Plus, ReceiptText, RefreshCw, Send, Settings2, UsersRound, WalletCards } from "lucide-react";
...
type FinancePage = "dashboard" | "invoices" | "payments-received" | "estimates" | "expenses" | "vendors" | "bills" | "banking" | "chart-of-accounts" | "drawings" | "payroll" | "inventory" | "purchase-orders" | "credit-notes" | "tax" | "documents" | "recurring" | "job-costing" | "time" | "budgets" | "assets" | "reports" | "audit" | "settings";
type InvoiceStatus = "Draft" | "Sent" | "Partial" | "Paid" | "Overdue";
type EstimateStatus = "Draft" | "Sent" | "Accepted" | "Declined";
type InvoicePayment = { id: string; date: string; amount: number; method: string; reference: string; account: string; currency?: PaymentCurrency; sourceAmount?: number; exchangeRate?: number; note: string; receivedBy: string };
type Invoice = { id: string; client: string; clinic: string; caseId?: string; caseNumber?: string; patient: string; issued: string; due: string; amount: number; paid: number; status: InvoiceStatus; service: string; payments: InvoicePayment[]; doctorAcceptedAt?: string };
...
const financeNav: { group: string; items: { id: FinancePage; label: string; icon: typeof LayoutDashboard; phase?: string }[] }[] = [
  ...
  { group: "Sales", items: [{ id: "invoices", label: "Invoices", icon: ReceiptText }, { id: "payments-received", label: "Payments received", icon: Banknote }, { id: "estimates", label: "Estimates", icon: FileCheck2 }, { id: "credit-notes", label: "Credit notes", icon: Files, phase: "2" }] },
...
function InvoiceModal({ invoice, doctors, onClose, onSubmit, onPayment }: { invoice: Invoice | null; doctors: string[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onPayment: (invoice: Invoice) => void }) {
  if (invoice) return <Modal title={invoice.id} subtitle={`${invoice.client} · ${invoice.patient}`} onClose={onClose}>
    <section className="finance-detail-modal invoice-detail-modal">
      <div className="invoice-detail-total"><span>Amount due</span><strong>{fmt(invoice.amount - invoice.paid)}</strong><Pill tone={invoice.status === "Paid" ? "good" : invoice.status === "Overdue" ? "danger" : "warning"}>{invoice.status}</Pill></div>
      <dl><div><dt>Service</dt><dd>{invoice.service}</dd></div><div><dt>Case</dt><dd>{invoice.caseNumber ? `#${invoice.caseNumber}` : "Manual invoice"}</dd></div><div><dt>Issued</dt><dd>{invoice.issued}</dd></div><div><dt>Due</dt><dd>{invoice.due}</dd></div></dl>
      {invoice.caseNumber && <button type="button" disabled className={`invoice-acceptance ${invoice.doctorAcceptedAt ? "accepted" : "pending"}`}>{invoice.doctorAcceptedAt ? <CheckCircle2 size={16} /> : <Hourglass size={16} />}<span><strong>{invoice.doctorAcceptedAt ? "Invoice accepted by doctor" : "Invoice awaiting doctor acceptance"}</strong><small>{invoice.doctorAcceptedAt ? invoice.doctorAcceptedAt.slice(0, 10) : "Awaiting confirmation in the doctor portal."}</small></span></button>}
      <div className="payment-history"><h3>Payment history</h3>{invoice.payments.length ? invoice.payments.map((payment) => <div key={`${payment.date}-${payment.amount}`}><span>{payment.date} · {payment.method}</span><b>{fmt(payment.amount)}</b></div>) : <p>No payment recorded.</p>}</div>
    </section>
    <div className="modal-actions invoice-detail-actions"><button className="secondary-button" type="button" onClick={onClose}>Close</button><button className="primary-button" type="button" disabled={invoice.status === "Paid"} onClick={() => onPayment(invoice)}>Record payment</button></div>
  </Modal>;
